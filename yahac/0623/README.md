...
## 관계형 데이터베이스 이론
대부분 알고 있는 내용  
LEFT JOIN에 대한 내용을 다시 한 번 복습할 수 있었음  

```sql
SELECT title, name FROM topic LEFT JOIN user ON topic.author = user.id;  
```
  
왼쪽에 적은 테이블을 기준으로 병합, 병합 기준은 topic의 author와 user의 id  
  
_JOIN의 종류에 따라 대응하지 않을 때 null로 채우기도 하고 아예 병합에서 제외하기도 함_  
...
**조회된 결과의 속성에 접근하는 방법 :**  
```php
$row의 title attribute에 접근 : $row['title']  
$row의 name attribute에 접근 : $row['name']  
$row의 description attribute에 접근 : $row['description']  
```
